...
async function buyNumber(provider, maxPrice) {
  if (provider === 'hero') {
    ...
    const res = await heroApi('getNumber', { service: 'ig', country: '187', maxPrice: price });
...
io.on('connection', (socket) => {
  ...
  socket.on('buy_number', async ({ provider, maxPrice } = {}) => {
    if (socket.role !== 'va') return;
    if (!botEnabled) { socket.emit('error_msg', 'Bot desactive - disponible de 12h a 22h.'); return; }
