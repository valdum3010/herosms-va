...
const GETATEXT_API_KEY = process.env.GETATEXT_API_KEY;
...
const GETATEXT_BASE = 'https://getatext.com/stubs/handler_api.php';
...
async function getatextApi(action, params = {}) {
  const url = new URL(GETATEXT_BASE);
  url.searchParams.set('api_key', GETATEXT_API_KEY);
  url.searchParams.set('action', action);
  for (const [k, v] of Object.entries(params)) url.searchParams.set(k, v);
  const resp = await fetch(url.toString());
  return await resp.text();
...
async function buyNumber(provider, maxPrice) {
  if (provider === 'hero') {
    const res = await heroApi('getNumber', { service: 'ig', country: '187', maxPrice: maxPrice || 0.80 });
  ...
  if (provider === 'smspool') {
    const res = await poolApi('getNumber', { service: 'ig', country: '187' });
  ...
  if (provider === 'bower') {
    ...
    console.log('SMSBower achat reponse:', res);
    if (res.startsWith('ACCESS_NUMBER:')) {
      const parts = res.split(':');
  ...
  if (provider === 'getatext') {
    ...
    if (res.startsWith('ACCESS_NUMBER:')) {
      const parts = res.split(':');
      return { ok: true, activationId: parts[1], number: parts[2], provider: 'getatext' };
    }
    if (res === 'OUT_OF_STOCK') return { ok: false, reason: 'Aucun numero disponible sur GetAText.' };
    if (res === 'INSUFFICIENT_FUNDS') return { ok: false, reason: 'Solde GetAText insuffisant.' };
    return { ok: false, reason: 'Erreur GetAText : ' + res };
  }
  if (provider === 'diddy') {
    try {
...
async function checkCode(provider, activationId) {
  ...
  if (provider === 'getatext') {
    const res = await getatextApi('getStatus', { id: activationId });
    console.log('GetAText status:', res);
    if (res.startsWith('STATUS_OK:')) return { ok: true, code: res.split(':')[1] };
    return { ok: false };
  }
  if (provider === 'bower') {
    const res = await bowerApi('getStatus', { id: activationId });
    console.log('SMSBower status:', res);
    if (res.startsWith('STATUS_OK:')) return { ok: true, code: res.split(':')[1] };
    return { ok: false };
  }
  if (provider === 'getatext') {
...
async function cancelNumber(provider, activationId) {
  try {
    ...
    if (provider === 'getatext') await getatextApi('setStatus', { id: activationId, status: 8 });
...
async function completeNumber(provider, activationId) {
  try {
    ...
    if (provider === 'getatext') await getatextApi('setStatus', { id: activationId, status: 6 });
  } catch(e) {}
}
...
io.on('connection', (socket) => {
  ...
  socket.on('buy_number', async ({ provider, maxPrice } = {}) => {
    if (socket.role !== 'va') return;
    if (!botEnabled) { socket.emit('error_msg', 'Bot desactive - disponible de 12h a 3h.'); return; }
    ...
    const providerName = provider === 'hero' ? 'HeroSMS' : provider === 'smspool' ? 'SMSPool' : provider === 'bower' ? 'SMSBower' : provider === 'getatext' ? 'GetAText' : 'DiddySMS';
    ...
    try {
      ...
      if (result.ok) {
        ...
        broadcastAll();
        if (result.provider === 'bower') {
          await bowerApi('setStatus', { id: result.activationId, status: 1 });
          setTimeout(() => startPolling(socket), 10000);
        } else {
          startPolling(socket);
        }
      } else {
